...
test('jede von einem Produkt genutzte Farbe existiert in der Lieferantentabelle', () => {
  ...
  for (const product of PRODUCTS) {
    const ref = supplierRefVon(product.id);
    if (!ref) continue;
    const map = getVariantMap(ref.supplierId);
    for (const color of product.colors) {
      if (!isColorSupported(map, color.id)) {
        missing.push(`${ref.supplierId}: Farbe "${color.id}" (Produkt ${product.id})`);
      }
    }
  }
  ...
  for (const product of PRODUCTS) {
    ...
    for (const size of product.sizes) {
      if (!isSizeSupported(map, size)) {
        missing.push(`${ref.supplierId}: Größe "${size}" (Produkt ${product.id})`);
      }
    }
  }
...
test('Produkte ohne Lieferantenzuordnung werden vollständig aufgelistet (auto-Aufgabe)', () => {
  ...
  const expected = PRODUCTS.filter((p) => !supplierRefVon(p.id))
    .map((p) => p.id)
    .sort();
